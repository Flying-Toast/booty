...
extern char default_exception_handler;
extern char default_pic_isr;
extern char asm_int21_wrapper;

static void install_isrs(void)
{
	for (size_t i = 0; i < sizeof(idt) / sizeof(idt[0]); ++i) {
		idt[i].offset_low = ((uintptr_t) &default_exception_handler) & 0xFFFF;
		idt[i].segment_selector = 0x08;
		idt[i].flags = 0x8E;
		idt[i].offset_high = ((uintptr_t) &default_exception_handler) >> 16;
	}

	for (size_t i = 0x20; i <= 0x2F; ++i) {
		idt[i].offset_low = ((uintptr_t) &default_pic_isr) & 0xFFFF;
		idt[i].offset_high = ((uintptr_t) &default_pic_isr) >> 16;
	}
